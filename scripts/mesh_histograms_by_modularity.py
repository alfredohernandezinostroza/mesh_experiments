...
def main(argv):
    if len(argv) < 2:
        print("Usage: python scripts/mesh_histograms_by_modularity.py path/to/graph.gexf [out_dir]")
        return 1
    gexf_path = Path(argv[1])
    if not gexf_path.exists():
        print(f"GEXF file not found: {gexf_path}")
        return 1
    out_dir = Path(argv[2]) if len(argv) > 2 else gexf_path.parent / "mesh_histograms"
